...
public class HeatGenerator : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (totaltime >= 13 && totaltime <= 13.5f && enemyCount <= 0)
        {
            SpawnEnemy();
        }
        else if (totaltime >= 26 && totaltime <= 26.5f && enemyCount <= 1)
        {
            SpawnEnemy();
        }
        else if (totaltime >= 31 && totaltime <= 31.5f && enemyCount <= 2)
        {
            SpawnEnemy();
        }
        else if (totaltime >= 37 && totaltime <= 44.5f && enemyCount <= 3)
        {
            SpawnEnemy();
        }
        else if (totaltime >= 44 && totaltime <= 54.5f && enemyCount <= 4)
        {
            SpawnEnemy();
        }
    }
